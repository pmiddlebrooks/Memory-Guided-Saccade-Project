function mem_list_units(subject,projectRoot, append, multiUnit)
%
...
% mem_sessions_<subject>.csv and save it into the appropriate data path for
...
fid=  fopen(fullfile(dataPath,['mem_sessions_',subject,'.csv']));
...
if append
    load(fullfile(dataPath, 'mem_units'), 'units')
...
for i = startInd :  length(sessionList)
    ...
    if neuronLogical(i)
        ...
        if multiUnit
            save(fullfile(dataPath, 'mem_multiunits'), 'units')
        else
            save(fullfile(dataPath, 'mem_units'), 'units')
        end
    end
end
